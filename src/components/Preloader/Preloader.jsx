...
const Preloader = ({ onAnimationComplete, ref }) => {
  ...
  useGSAP(
    () => {
      ...
      try {
        const textElement = wrapperRef.current.querySelector("h1");
        
        sentenceSplit = new SplitType(".preloader-sentence h1", {
          types: "chars, words",
          charClass: "char",
        });

        const targetElements = sentenceSplit.chars || [];
        gsap.set(targetElements, { opacity: 0, y: 20 });
        gsap.set(textElement, { opacity: 1 }); // Reveal parent now that children are hidden
        ...
        const startExit = () => {
          ...
          exitTimeline.to(targetElements, {
            opacity: 0,
            y: -20,
            stagger: 0.015,
            duration: 0.5,
        ...
        prepareForUnmountRef.current = () => {
          ...
          if (sentenceSplit) {
            gsap.set(textElement, { opacity: 0 }); // Hide before revert to prevent flash
        ...
        introTimeline.to(targetElements, {
          opacity: 1,
          y: 0,
          stagger: 0.02,
          ease: "back.out(1.2)",
  ...
  return (
    <div className="preloader-wrapper" ref={wrapperRef}>
      ...
      <div className="preloader-sentence">
        <h1 style={{ opacity: 0 }}>Crafting systems that feel alive</h1>
      </div>
    </div>
  );
};
